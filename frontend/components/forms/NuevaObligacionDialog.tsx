...
import { createObligation } from "@/services/mutations";
...
export function NuevaObligacionDialog() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const type = fd.get("type") as ObligationType;

    if (!concept) { setError("El concepto es obligatorio"); return; }
    if (!amount || amount <= 0) { setError("El monto debe ser mayor a 0"); return; }
    if (!dueDate) { setError("La fecha de vencimiento es obligatoria"); return; }

    setLoading(true);
    try {
      await createObligation({ concept, amount, dueDate, type });
      setSuccess(true);
      router.refresh();
      setTimeout(() => close(), 700);
    } catch (err) {
      const msg = err instanceof ApiError ? err.message : "Error al registrar obligación";
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-[0.95rem] font-bold text-neutral-900">Nueva Obligación</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          <div className="flex flex-col gap-1.5">
            ...
            <input name="concept" required placeholder="Ej: Alquiler campo Norte" className={INPUT_CLS} />
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select name="type" defaultValue="RENT" className={INPUT_CLS}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="dueDate" type="date" required className={INPUT_CLS} />
            </div>
          </div>
